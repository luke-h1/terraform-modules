data "archive_file" "log_exporter" {
  ...
  output_path = "${path.module}/lambda/tmp/"
}
...
resource "aws_lambda_function" "log_exporter" {
  ...
  handler          = "cloudwatch-to-s3.handler"
...
resource "aws_cloudwatch_event_rule" "log_exporter" {
  name                = "log-exporter-cron-${random_string.random.result}"
  description         = "The cron to export logs to S3"
